...
/// Setup Parameters 
  ...
  cl_mem output_buf;
  cl_int errcode = 0;                                          // num_devices elements
  int status;
  cout << getGaussianKernel(3,0)<< endl;
///Setup CL program and kernel
  ...
  unsigned char **opencl_program = read_file("tiledmatrix_mult.cl");
  ...
  kernel = clCreateKernel(program, "tiledmatrix_mult", &errcode);
...
// CPU PART
  ...
  // Multiply Matrix /!\ if you do that after the GPU activity without having unmapped, data is corrupted and you don't get the good result 
  clock_gettime(clk_id, &start);
  int k;
  for (int i = 0; i < M; ++i)
  {
    ...
    {
      float acc = 0.0f;
      for (k = 0; k < K; k++) {acc += input_a[i * M  +  k] * input_b[k * K + j];}
...
// KERNEL PART
  ...
  const size_t local_work_size[2] = {8,8}; //Change values here to change work group size

  clock_gettime(clk_id, &start);
  status = clEnqueueNDRangeKernel(queue, kernel, 2, NULL,
                                  global_work_size, local_work_size, 2, write_event, &kernel_event); ///THERE you launch the kernels
...
/* PERFORMANCES of the matrix multiplication for N = M = K = 256

CPU took 0.148722538 seconds to compute.

Without Work groups : 
GPU took 0.064043522 seconds to compute.

With 4*4 work groups :
GPU took 0.017829533 seconds to compute.

With 8*8 work groups : 
GPU took 0.014087700 seconds to compute.

With 16*16 work groups :
GPU took 0.012559037 seconds to compute. */
